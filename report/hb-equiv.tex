...
By IH on $\T_1$ with $\C$ we get \trahb{\hbctx}{\sub{\benv}{\bval}{\var} \envcons \othervar}{\bexp_1}{\sub{\hexp_1}{\hval}{\var}} (by $\T_1'$) which has the right shape to finish the case with \rulename{T-Lam}.
...
\begin{theorem}[Equivalence \textnormal{\hlang}-\textnormal{\blang}]
\label{thm:equivalence-hb} If \trahb{\hbctx}{\envnil}{\bexp}{\hexp}, then \hev{\hexp}{\n{\nat}} if and only if \bev{\envnil}{\bexp}{\n{\nat}}.
\end{theorem}

\Twelf
% something about the fact that we cannot state equivalence in twelf.
...
% \paragraph{Case \textnormal{\rulename{Tv-Here}}}
...
% We now get the required derivation $\Bv$ of $\hevv{\sg', \v}{\z}{\v}$ directly by rule \rulename{Bv-Here}.
% And since $\svalsi(x) = \svalsi'[x \mapsto \c](x) = \c$, we can take $\C = \C'$.

% \paragraph{Case \textnormal{\rulename{Tv-There}}}
...
% $\T$ must end in \rulename{T-Num} and so $\b = \n{n}$.
% We have $\subs{\e}{\svalsi} = \n{n}$, so $\E$ must end in \rulename{E-Num} and $v = \n{n}$.
% Taking $\v = \n{n}$, by rule \rulename{B-Num} we get a derivation $\B$ of $\hev{\sg}{\n{n}}{\n{n}}$ as required.
% And $\cor{\n{n}}{\n{n}}$ by rule \rulename{C-Num}.
...
% \end{prooftree}
% So $\b = \lam{}{\b_1}$. We have $\subs{\e}{\svalsi} = \subs{(\lam{x}{\e_1})}{\svalsi} = \lam{x}{\subs{\e_1}{\svalsi \wo x}}$, so $\E$ must end in \rulename{E-Lam} and $v = \lam{x}{\subs{\e_1}{\svalsi \wo x}}$.
% Taking $\v = \cl{\sg}{\b_1}$, we get the required $\B$ showing $\hev{\sg}{\lam{}{\b_1}}{\cl{\sg}{\b_1}}$ by rule \rulename{B-Lam}.
...
% We have $\subs{\e}{\svalsi} = \subs{\e_1}{\svalsi} \app \subs{\e_2}{\svalsi}$, so $\E$ must end in \rulename{E-App} and have the form
% \begin{prooftree}
...
% \paragraph{Case \textnormal{\rulename{B-Num}}}
...
% $\T$ must end in \rulename{T-Num} and so $\e = \n{n}$.
% Since $\subs{\e}{\svalsi} = \n{n}$, we can take $\c = \n{n}$ and get the required derivation $\E$ of $\ev{\n{n}}{\n{n}}$ by rule \rulename{E-Num}.
% And $\cor{\n{n}}{\n{n}}$ by rule \rulename{C-Num}.

% \paragraph{Case \textnormal{\rulename{B-Var}}}
...
% \paragraph{Case \textnormal{\rulename{B-Lam}}}
...
% \end{prooftree}
% So $\e = \lam{x}{\e_1}$.
% We have $\subs{\e}{\svalsi} = \subs{(\lam{x}{\e_1})}{\svalsi} = \lam{x}{\subs{\e_1}{\svalsi \wo x}}$, and taking $\c = \lam{x}{\subs{\e_1}{\svalsi \wo x}}$ we get the required $\E$ showing $\ev{\lam{x}{\subs{\e_1}{\svalsi \wo x}}}{\lam{x}{\subs{\e_1}{\svalsi \wo x}}}$ by rule \rulename{E-Lam}.
% We then construct the required $\C$ using rule \rulename{C-Fun} as follows:
% \begin{prooftree}
%   \prem{\D}{\comp{\sg}{\De}{\svalsi}}
%   \prem{\T_1}{\tra{\De, x}{\e_1}{\b_1}}
%   \binf{\cor{\cl{\sg}{\b_1}}{\lam{x}{\subs{\e_1}{\svalsi \wo x}}}}
